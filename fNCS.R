...
participants=100
trials=160
b<-setNames(data.frame(matrix(ncol=7, nrow=0)), c("state","g9","g15","g16","E","K","f"))

for (loop in 1:1000){
  for (Miu in seq(from=10.46, to=10.46, by=1)){
    for (E in seq(from=55.47, to=55.47, by=1)){
      ...
      rand_vec_s1<-c()
      rand_vec_s2<-c()
      Miu_shuffled$State_Shuffled<-Miu_shuffled$State
      
      for (i in 1:nrow(Miu_shuffled)){
        rand_vec_s1[i] <- sample (c(4,6,7), size=1, replace=F)
        rand_vec_s2[i] <- sample (c(2,3,5), size=1, replace=F)
        Miu_shuffled$State_Shuffled[i] <- ifelse((Miu_shuffled$State[i] == 4 |Miu_shuffled$State[i] == 6 | Miu_shuffled$State[i] == 7), 
                                                 rand_vec_s1[i], Miu_shuffled$State_Shuffled[i])
        Miu_shuffled$State_Shuffled[i] <- ifelse((Miu_shuffled$State[i] == 2 |Miu_shuffled$State[i] == 3 | Miu_shuffled$State[i] == 5), 
                                                 rand_vec_s2[i], Miu_shuffled$State_Shuffled[i])
      }
...
write.csv(b,"fNCS game.csv", row.names = FALSE)
library(beepr)
beep(sound=2)
